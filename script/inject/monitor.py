...
class ItemsNamespace(BaseNamespace, BroadcastMixin):
    AllConnected = []
    ...
    def recv_disconnect(self):
        try:
            ItemsNamespace.AllConnected.remove(self)
        except:
            pass
        print 'disconnect, there are currently', len(ItemsNamespace.AllConnected), ' connected clients'

    def recv_connect(self):
        if len(ItemsNamespace.AllConnected) == 0:
            self.spawn(ItemsNamespace.sendItems)
        ItemsNamespace.AllConnected.append(self)
        print 'connect, there are currently', len(ItemsNamespace.AllConnected), ' connected clients'

        self.emit('update', lastTotalPacket)
...
def progress(environ):
    ...
    if environ['REQUEST_METHOD'] == 'POST':
        ...
        if 'completed' in parms:
            ...
            parms['bar'] = 'Total'
            lastTotalPacket = parms

        if 'item' in parms:
            parms['bar'] = parms['item']

        itemEvent.put_nowait(parms)
        pass

    if environ['REQUEST_METHOD'] == 'GET':
        pass

    #print "xxxxxxxxxx", environ
    return 'hihi'
...
function addBar(id, collectionName) {
    ...
    addText.appendChild(document.createTextNode(''));
...
function updateBar(id, newVal, addText, collectionName) {
    ...
    var allBars = document.getElementById('bars');

    if(elm.id == 'Total') {
      allBars.insertBefore(elm, allBars.childNodes && allBars.childNodes.length && allBars.childNodes[0] || null)
    } else {
      if(allBars.childNodes && allBars.childNodes.length > 1) {
        allBars.insertBefore(elm, allBars.childNodes[1]);
      } else {
        allBars.appendChild(elm);
      }
    }

    bar[0].style.width = newVal + '%'

    var addTextNode = elm.getElementsByClassName('add-text');
    addTextNode[0].innerHTML = addText;
}
...
var socket = io.connect('/items');
socket.on('update', function(update) {
    console.log(update);
    document.getElementById("body");
    if(update.query) {
       document.getElementById('query').innerHTML = update.query;
    }
    if(update.completed) {
...
class Application(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if path == '':
            return rootPage(start_response)

        if path == 'socket.io.js':
            return socketIo(start_response)

        if path.startswith('static/') or path == 'index.html':
            #path = 'IaGbInject/%s' % path
        ...
        if path.startswith("socket.io"):
            socketio_manage(environ, {'/items': ItemsNamespace})
            return []
        elif path == 'progress':
            start_response('200 OK', [('Content-Type', 'application/json')])
            return [progress(environ)]
        elif path == 'shutdown':
            sys.exit(0)
...
def main():
    SocketIOServer(('0.0.0.0', 10008), Application(),
                   resource="socket.io", policy_server=False).serve_forever()




def socketIo(start_response):
