...
# need to change in changelog too!! dont' forget!!!
	git archive --format=tar --prefix=gb-1.15/ master > ../gb_1.15.orig.tar
	rm -rf debian
# change "-p gb_1.0" to "-p gb_1.1" to update version for example
	dh_make -e gigablast@mail.com -p gb_1.15 -f ../gb_1.15.orig.tar
...
# alien it
	sudo alien --to-rpm gb_1.15-1_i386.deb
# upload rpm
