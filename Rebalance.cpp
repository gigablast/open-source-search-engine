...
//   rebalance?
void CommandRebalance ( char *rec ) {
...
char *Rebalance::getNeedsRebalance ( ) {
	...
	if ( ! g_parms.m_inSync ) return NULL;

	// wait for all hosts to agree
	if ( ! m_hostsConfInAgreement ) return NULL;
	...
	// can only occur once m_hostsConfInAgreement is true.
	SafeBuf sb;
	...
	if ( x != g_hostdb.m_myHost->m_shard ) m_needsRebalance = true;
...
void Rebalance::scanLoop ( ) {
	...
	for ( ; m_collnum < g_collectiondb.m_numRecs ; m_collnum++ ) {
		...
		for ( ; m_rdbNum < m_numRdbs ; m_rdbNum++ ) {
			// skip if not good
			Rdb *rdb = g_process.m_rdbs[m_rdbNum];
			// not an RDB2
			if ( rdb->m_isSecondaryRdb ) continue;
...
void Rebalance::gotList ( ) {
	...
	for ( ; ! m_list.isExhausted() ; m_list.skipCurrentRec() ) {
		...
		// get shard
		long shard = getShardNum ( rec , rdb->m_rdbId );
		// skip it if it belongs with us
		if ( shard == myShard ) continue;
		// otherwise, it does not!
		//m_metaList.pushChar ( rdb->m_rdbId );
		m_posMetaList.addRec ( rec , recSize );
		// make it negative
		keyPtr[0] &= 0xfe;
		// delete from us
		m_negMetaList.storeRec ( myShard , keyPtr );
		//m_metaList.safeMemcpy ( keyPtr );
	}
